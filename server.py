...
async def update_huwise_dataset(
    ...
    """
    Récupère tous les records Grist via pagination et met à jour le dataset Huwise.
    ...
    async with httpx.AsyncClient() as client:
        while True:
            response = await client.get(
                f"{grist_url}?limit={limit}&offset={offset}",
    ...
    async with httpx.AsyncClient() as client:
        response = await client.post(
            ...
            headers={
                "Authorization": f"Apikey {huwise_token}",
